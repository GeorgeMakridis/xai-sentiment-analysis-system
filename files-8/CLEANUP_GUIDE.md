...
|-----------|--------|-------|---------|
| ai_outputs/app.py | 2,321 lines | 2,011 lines | 310 lines |
| dashboard/app.py | 523 lines | 338 lines | 185 lines |
| xai_service/app.py | 5,993 lines | 1,806 lines | 4,187 lines |
...
### 1. News Sentiment JSON ingestion (NameError)
`load_news_sentiment_data()` was deleted during cleanup but still called from
`load_data()`. Restored the function — it flattens the nested JSON structure
(date → company → articles) into a DataFrame with title/asset/sentiment columns.

### 2. Timeseries preprocessing (NameError)
`preprocess_timeseries_data()` was never defined in any commit but called from
`ingest_data()`. Added a proper implementation that detects date columns, creates
lag/rolling features, and identifies the target column.

### 3. Removed dead xlsx/parquet branches
`load_data()` had `.xlsx` and `.parquet` branches, but those dependencies
(openpyxl, pyarrow) were removed. Cleaned to support only CSV and JSON.

## New Feature: Dataset Persistence to RustFS

Previously uploaded datasets lived only in `shared_volume/uploads/` (lost on
container restart). Now they're also persisted to the remote RustFS instance.

### New endpoints
- `POST /api/store-dataset` — reads from shared_volume, uploads to RustFS `xai-datasets/{user}/{filename}`
- `POST /api/fetch-dataset` — downloads from RustFS to shared_volume so xai_service can ingest

### Updated dashboard flow
- **Upload**: after successful ingest, dashboard calls `/api/store-dataset` to persist
- **Select (source=restfs)**: dashboard calls `/api/fetch-dataset` to download first, then ingests

### Updated storage map
| Bucket | Contents | When Written |
|--------|----------|--------------|
| `xai-datasets` | `{user}/{filename}` | On every file upload |
| `xai-plots` | `{user}/{plot_id}.png` | On data-statistics |
| `xai-metadata` | `{user}/{plot_id}.json` | On data-statistics |
| `xai-metadata` | `{user}/plots_registry.json` | On data-statistics |
...
| ai_outputs | `POST /api/store-dataset` | Persist uploaded file to RustFS |
| ai_outputs | `POST /api/fetch-dataset` | Download RustFS dataset to local |
